 ...
 * persists it, but the underlying transports never read it: `MCPClientManager`
 * forwards the transport options verbatim to `StreamableHTTPClientTransport` /
 * `SSEClientTransport`, whose options types have no `headers` member and whose
 * constructors only merge `requestInit` into outbound fetches
 * (`createFetchWithInit(opts?.fetch, opts?.requestInit)`).
 *
