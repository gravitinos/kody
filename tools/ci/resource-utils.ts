 ...
 *
 * Environments with `APP_BASE_URL` but no `PACKAGE_APP_BASE_URL` publish only
 * the first-party app custom domain. Hosted package-app requests then keep
 * failing closed in production instead of forcing self-hosters to provision a
 * second registrable domain for a surface they do not use. Environments with
 * neither origin (preview, test) publish no routes at all.
 */
function addPackageAppCustomDomainRoute(input: {
	...
	if (!packageAppHostname && !appHostname) return
	if (packageAppHostname && !appHostname) {
		return fail(
	...
	if (appHostname && appHostname === packageAppHostname) {
		return fail(
	...
	if (!packageAppHostname && packageAppLegacyHostnames.length > 0) {
		return fail(
			`wrangler config "${input.baseConfigPath}" sets "env.${input.envName}.vars.PACKAGE_APP_LEGACY_HOSTS" without "PACKAGE_APP_BASE_URL". Remove the legacy hosts or configure the canonical hosted package-app origin.`,
		)
	}
	...
	const packageAppZoneName = packageAppHostname
		? readPackageAppZoneName(packageAppHostname)
		: null
	if (packageAppHostname && !packageAppZoneName) {
		return fail(
	...
	const wildcardRoutePattern = packageAppHostname
	...
	if (!packageAppHostname) {
		console.error(
			`Worker routes: ${appHostname} (APP_BASE_URL)${legacyNote}; workers.dev trigger kept. Hosted package apps are disabled because PACKAGE_APP_BASE_URL is unset.`,
		)
		return
	}
	console.error(
